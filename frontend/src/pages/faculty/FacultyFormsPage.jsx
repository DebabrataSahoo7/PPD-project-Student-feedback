...
export default function FacultyFormsPage() {
  ...
  useEffect(() => {
    api.get('/forms').then(r => setForms(r.data.data)).finally(() => setLoading(false));
  }, []);

  if (loading) return <FullPageSpinner />;

  return (
    <PageLayout title="My Forms">
      <div className="space-y-3">
        {forms.length === 0 ? (
          <EmptyState icon="auto_stories" title="No forms assigned" description="Forms linked to your subjects will appear here." />
        ) : forms.map(f => (
